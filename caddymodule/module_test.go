...
// miss_fallthrough parses true/false onto MissFallthrough. Default
// false: a cache miss returns scopecache's own miss response. When
// true, a miss is handed to the next handler in the Caddy chain.
func TestUnmarshalCaddyfile_MissFallthrough(t *testing.T) {
	t.Run("true", func(t *testing.T) {
		...
		if !h.MissFallthrough {
			t.Errorf("MissFallthrough=false, want true")
		}
	})
	...
	t.Run("false", func(t *testing.T) {
		...
		if h.MissFallthrough {
			t.Errorf("MissFallthrough=true, want false")
		}
	})
	...
	t.Run("unset defaults to false", func(t *testing.T) {
		...
		if h.MissFallthrough {
			t.Errorf("MissFallthrough=true, want false when unset")
		}
	})

	t.Run("rejects non-bool", func(t *testing.T) {
		input := `scopecache {
			miss_fallthrough maybe
		}`
		var h Handler
		if err := h.UnmarshalCaddyfile(caddyfile.NewTestDispenser(input)); err == nil {
			t.Fatal("expected error for miss_fallthrough=maybe; got nil")
		}
	})
}
